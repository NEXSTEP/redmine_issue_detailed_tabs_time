  ...
  module Patches
    ...
    module IssuesHelperPatch
      ...
      module InstanceMethods
        ...
        def validTabs 
          ['history_all','history_comments','history_activity','history_private','history_time']
        end
        ...
        def get_issue_history_tabs(entries, journals)
          ...
          tabs.push({:label => :label_tab_history_all, :name => 'history_all'}) if User.current.allowed_to?(:view_all,@project,:global => true)
          for entry in entries
            if entry.is_a?(Journal) && journals.include?(entry) 
              tabs.push({:label => :label_tab_history_comments, :name => 'history_comments'}) if entry.notes? && User.current.allowed_to?(:view_comments,@project,:global => true)
              tabs.push({:label => :label_tab_history_private, :name => 'history_private'} ) if entry.private_notes?
              tabs.push( {:label => :label_tab_history_activity, :name => 'history_activity'}) if (entry.details.any?) && User.current.allowed_to?(:view_activity,@project,:global => true)
            elsif entry.is_a?(TimeEntry) && User.current.allowed_to?(:view_time_entries, @project) 
              tabs.push( {:label => :label_tab_history_time, :name => 'history_time'})    
            end
          end 
          tabs
        end
        ...
        def get_selected_issue_history_tab(tabs)
          ...
          def_tab = User.current.pref.def_history_tab
          
          return selected_tab ||= def_tab if tabsContainContent.include? def_tab
...
IssuesHelper.send(:include,  IssueDetailedTabs::Patches::IssuesHelperPatch)
